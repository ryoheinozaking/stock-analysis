...
月次スナップショットで成長株パイプラインを再実行し、
各ファンダ指標の Rank IC（予測力）・Top N α・重みスイープを出力する。

Usage:
    .venv\\Scripts\\python.exe diagnose_growth.py
    .venv\\Scripts\\python.exe diagnose_growth.py --fwd 90 --top 20
    .venv\\Scripts\\python.exe diagnose_growth.py --fwd 60 --snapshots 12  # 直近12ヶ月のみ

出力 CSV:
    data/diagnose_growth/ic_by_factor.csv   -- Rank IC 集計
    data/diagnose_growth/alpha_by_topn.csv  -- Top N 別 α
    data/diagnose_growth/weight_sweep.csv   -- 重みスイープ
"""
...
from services.diagnose_growth_service import run_diagnosis
...
def print_summary(result: dict) -> None:
    ...
    # バリューモード比較
    print(f"\n  参考 [バリューモード実績 (37スナップショット)]:")
    print(f"  {'PBR':<18} {'IC=+0.151':>8}  37/37 (100%)  <- 最強")
    print(f"  {'PSR':<18} {'IC=+0.143':>8}  高頻度プラス")
    print(f"  {'ROE':<18} {'IC=-0.027':>8}  逆効果 -> ハードフィルタ撤廃済み")
    print(f"  {'rev_growth':<18} {'IC=-0.017':>8}  逆効果")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="成長株モード診断（Rank IC / Top N α / 重みスイープ）"
    )
    parser.add_argument(
