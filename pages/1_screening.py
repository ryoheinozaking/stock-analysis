...
max_stocks = st.sidebar.slider("対象銘柄数上限", min_value=50, max_value=500, value=200, step=50)
...
if not run_button and st.session_state["screening_result"] is None:
    ...
    1. **左サイドバー**でスクリーニング条件を設定します
    2. **「▶ スクリーニング実行」** ボタンをクリックして実行します
    3. 結果一覧から銘柄を選択して詳細ページで分析できます
...
if run_button:
    ...
    with st.status("スクリーニング実行中...", expanded=True) as status:
        try:
            # 銘柄リスト取得
            st.write("銘柄マスタを取得中...")
            ...
            st.write(f"対象銘柄数: {len(codes)} 件")

            # JQuantsClient インスタンス生成
...
if st.session_state["screening_result"] is not None:
    ...
    if result_df.empty:
        ...
        # 追加フィルター適用
        if volume_surge and "avg_volume" in result_df.columns and "volume_avg_min" in result_df.columns:
            result_df = result_df[result_df["avg_volume"] > result_df["volume_avg_min"] * 2]
        elif volume_surge and "avg_volume" in result_df.columns:
        ...
        if near_52w_high and not result_df.empty:
            from services.jquants_service import get_ohlcv
            from datetime import datetime, timedelta
        ...
        if selection and selection.selection and selection.selection.rows:
            ...
            selected_code = result_df.iloc[selected_row_idx]["code"]   # 内部では5桁
