...
>
> このファイル（`stock_analysis/AGENTS.md`）は stock_analysis 固有の技術詳細のみを記載する。Claude Code 側の `stock_analysis/CLAUDE.md` と同期して運用する。
...
ANTHROPIC_API_KEY=...    # Codex API（Haiku-4-5）
...
│   ├── 5_portfolio_analysis.py   # AI分析（Codex APIによる総評・銘柄別売買提案）
...
│   ├── claude_service.py         # Codex API統合（ポートフォリオ分析・IR要約）
│   ├── ir_service.py             # 適時開示フィルタリング（3層分類）・PDF抽出
│   ├── portfolio_service.py      # SBI証券CSV パーサー（CP932デコード）
│   ├── pipeline_service.py       # パイプライン本体（ハードフィルタ→スコアリング→Codex分析）
...
3_disclosures.py（Tab2）
  ...
  → claude_service.summarize_ir()（Codex API → ir_summaries.json保存）
```
...
| Codex API | `api.anthropic.com` | `ANTHROPIC_API_KEY` | AI分析・IR要約 |
| SBI証券CSV | ローカル | 不要 | ポートフォリオ保有データ |
| TDnet PDF | `document_url`経由 | 不要 | 決算短信PDF本文 |

### Codex API 設定
- モデル: `Codex-haiku-4-5-20251001`
...
AGENTS.md/コードに散らばっていた「権利修正済: AdjO/H/L/C/Vo を使用」の記述は **単発スナップショット利用時のみ妥当**。長期蓄積データには当てはまらない。
