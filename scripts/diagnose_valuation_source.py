...
def main():
    ...
    snaps = [s for s in generate_monthly_snapshots(prices["Date"].min(), prices["Date"].max(), fwd) if s >= JPX_START]
    ...
    for name in variants:
        ...
        for i, snap in enumerate(snaps):
            ...
            print(f"[{name}] {i + 1}/{len(snaps)} {snap} {status}（経過 {time.time() - t0:.0f} 秒）", flush=True)
